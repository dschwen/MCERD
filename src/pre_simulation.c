 ...
 */   
   ...
   rotate(detector->angle,PI,theta_lab,fii_lab,&theta,&fii);
 ...
 */
   ...
   sprintf(fname,"%s%s",global->basename,FPRE);
   fp = fopen(fname,"w");
   if(fp == NULL){
      /* fatal_error("Could not open file for presimulation output\n"); */
      error_dialog("Could not open file for presimulation output");
...
void fit_linear(double *x,double *y,int n,double *a,double *b)
{
   ...
   if(n < 3){
      ...
      error_dialog("Too few points in the LS-fit in the pre-simulation analysis");
      longjmp(err_buf,1);
   }
